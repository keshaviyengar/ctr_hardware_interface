...
from CTR_Python.Segment import Segment

NUM_TUBES = 3


class CTRModel(object):
    def __init__(self, ctr_parameters):
    ...
    def forward_kinematics(self, joint, **kwargs):
        """
    ...
    def ctr_model(self, uz_0, alpha_0, r_0, R_0, segmentation, beta):
        ...
        """
        tube1 = self.ctr_parameters[0]
        tube2 = self.ctr_parameters[1]
        tube3 = self.ctr_parameters[2]
        Length = np.empty(0)
        r = np.empty((0, 3))
        u_z = np.empty((0, 3))
        alpha = np.empty((0, 3))
